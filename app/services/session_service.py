...
class SessionService:
    ...
    async def set_excel_file(
        ...
        result = await collection.find_one_and_update(
            {"session_id": session_id},
            {"$set": {"excel_file": excel_file.model_dump(), "updated_at": datetime.utcnow()}},
            return_document=True
        )
    ...
    async def advance_row(self, session_id: str) -> Optional[int]:
        ...
        await self.update_session(session_id, SessionUpdate(current_row=new_row))
    ...
    async def update_audio_transcription(
        ...
        result = await collection.find_one_and_update(
            ...
            {"$set": {"transcription": transcription.model_dump(), "status": AudioStatus.TRANSCRIBED.value}},
            return_document=True
        )
        if result:
            result["_id"] = str(result["_id"])
    ...
    async def get_parsed_row(self, session_id: str, row_number: int) -> Optional[ParsedRow]:
        collection = get_parsed_rows_collection()
        doc = await collection.find_one({"session_id": session_id, "row_number": row_number})
        if doc:
            doc["_id"] = str(doc["_id"])
    ...
    async def confirm_row(self, session_id: str, row_number: int, final_data: Dict[str, Any]) -> Optional[ParsedRow]:
        return await self.update_parsed_row(
            session_id, row_number,
            ParsedRowUpdate(final_data=final_data, status=RowStatus.CONFIRMED)
        )

    async def mark_row_written(self, session_id: str, row_number: int) -> Optional[ParsedRow]:
        collection = get_parsed_rows_collection()
        result = await collection.find_one_and_update(
            {"session_id": session_id, "row_number": row_number},
            {"$set": {"status": RowStatus.WRITTEN.value, "written_to_excel": True, "updated_at": datetime.utcnow()}},
            return_document=True
        )
        if result:
            result["_id"] = str(result["_id"])
    ...
    async def log_error(
        ...
        error_data["_id"] = str(result.inserted_id)
        logger.error(f"Error logged for session {session_id}: {error_type.value} - {error_message}")
        return ErrorLog(**error_data)

    async def get_session_errors(self, session_id: str, unresolved_only: bool = True) -> List[ErrorLog]:
        collection = get_error_logs_collection()
    ...
    async def get_session_stats(self, session_id: str) -> Dict[str, Any]:
        ...
        draft_count = await parsed_collection.count_documents({"session_id": session_id, "status": RowStatus.DRAFT.value})
        confirmed_count = await parsed_collection.count_documents({"session_id": session_id, "status": RowStatus.CONFIRMED.value})
        written_count = await parsed_collection.count_documents({"session_id": session_id, "status": RowStatus.WRITTEN.value})
        error_count = await error_collection.count_documents({"session_id": session_id, "resolved": False})
